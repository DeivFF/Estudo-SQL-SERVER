...
-- Essa tabela é a tabela dimensão, a tabela dimensão sempre possuirá uma primary key ou seja, chave identificadora, a tabela dimensão possui informações/caracterísicas sobre determinado dado que terá ligação a tabela fato.
...
-- Essa tabela é a tabela fato, a tabela fato possui informações de eventos e sempre possuirá chave estangeira, é possível que uma tabela dimensão possua chave estangeira também, mas sempre a fato possuirá necessariamente, a sintaxe é: FOREIGN KEY (COLUNA_DA_TABELA_FATO_QUE_POSSUI_IDENTIFICADOR_NA_OUTRA_COLUNA) REFERENCES TABELA_QUE_POSSUI_A_PRIMARY_KEY(COLUNA_QUE_TEM_A_PRIMARY_KEY)
